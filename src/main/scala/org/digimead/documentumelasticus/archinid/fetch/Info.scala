...
import org.digimead.documentumelasticus.archinid.bot.IRCEntity
import org.digimead.documentumelasticus.archinid.bot.JabberEntity
import org.digimead.documentumelasticus.archinid.{ Info => HInfo, InfoAkka, InfoIRC, InfoJabber }
...
object Info {
  ...
  def run(config: Config, Pool: PoolSingleton, timeout: Long = defaultTimeout) = synchronized {
    ...
    val subscriber = new Subscriber[pool.Event, Pool.Pub] {
      def notify(pub: Pool.Pub, event: pool.Event): Unit = {
        event match {
